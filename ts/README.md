...
The TypeScript SDK for the Sumo API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const result = await client.Basho().list()
...
```ts
const result = await client.Basho().load({ id: 'example_id' })
...
```ts
const client = new SumoSDK({ apikey: '...' })
