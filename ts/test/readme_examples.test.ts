...
//     (\`new SumoSDK(...)\` / \`SumoSDK.test(...)\` become
//     \`SumoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"basho":{"test01":{"id":"test01"}},"kimarite":{"test01":{"id":"test01"}},"measurement":{"test01":{"id":"test01"}},"rank":{"test01":{"id":"test01"}},"rikishi":{"test01":{"id":"test01"}},"shikona":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SumoSDK(...)\` and any
// \`SumoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SumoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SumoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
