...
describe('RikishiDirect', async () => {
  ...
  test('direct-load-rikishi', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-rikishi', setup.live)) return
    if (skipIfMissingIds(t, setup, ["opponent01"])) return
  ...
  test('direct-list-rikishi', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-rikishi', setup.live)) return
    if (skipIfMissingIds(t, setup, ["rikishi01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SUMO_TEST_RIKISHI_ENTID']
