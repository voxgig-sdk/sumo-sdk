...
describe('BashoDirect', async () => {
  ...
  test('direct-load-basho', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-basho', setup.live)) return
    if (skipIfMissingIds(t, setup, ["division01"])) return
  ...
  test('direct-list-basho', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-basho', setup.live)) return
    if (skipIfMissingIds(t, setup, ["basho01","day01","division01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SUMO_TEST_BASHO_ENTID']
