...
from sumo_sdk.sumo_types import (
    Kimarite,
    KimariteLoadMatch,
    KimariteListMatch,
)
...
class KimariteEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return KimariteEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Kimarite:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Kimarite().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Kimarite]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Kimarite().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
