...
class SumoTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SumoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SumoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SumoTestRunner.getenv("SUMO_TEST_LIVE")
        override = SumoTestRunner.getenv("SUMO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SumoTestRunner.getenv(key)
        ...
        explain = SumoTestRunner.getenv("SUMO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SUMO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SumoTestRunner._test_control is not None:
            return SumoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SumoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SumoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SumoTestRunner.load_test_control()
...
def load_env_local():
    SumoTestRunner.load_env_local()


def env_override(m):
    return SumoTestRunner.env_override(m)


def entity_data(v):
    return SumoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SumoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SumoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SumoTestRunner.load_test_control()


def live_client_options():
    return SumoTestRunner.live_client_options()


def live_delay_ms():
    return SumoTestRunner.live_delay_ms()
