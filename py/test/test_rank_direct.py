...
class TestRankDirect:

    def test_should_direct_list_rank(self):
        setup = _rank_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-rank", "live" if setup["live"] else "unit")
        if _skip:
...
def _rank_direct_setup(mockres):
