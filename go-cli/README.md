...
boru-driven command-line client **and** interactive REPL for the Sumo
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sumo-cli)
make build
...
./sumo-cli list basho
./sumo-cli load 1 basho            # {id:1} shorthand
./sumo-cli load '{id:1}' basho       # explicit match map
./sumo-cli list kimarite

# 5. Override the API base URL for a single call
SUMO_BASE=https://api.example.com ./sumo-cli list basho
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sumo-cli
   ```
...
```sh
./sumo-cli load 1 basho          # scalar shorthand for {id:1}
./sumo-cli load '{id:1}' basho     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `sumo>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `basho`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sumo-cli`. |
...
(`./sumo-cli <expr>`) and interactively (the REPL), and expressions compose the
