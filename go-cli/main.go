// Command Sumo-cli is a CLI + REPL for the Sumo SDK,
...
const entitiesHelp = "basho kimarite measurement rank rikishi shikona"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SUMO_APIKEY (api key), SUMO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SUMO_APIKEY carries the API key and
	// SUMO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SUMO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SUMO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSumoSDK(opts)
