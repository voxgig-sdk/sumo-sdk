...
The PHP SDK for the Sumo API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Basho(null)->list(null, null);
...
```php
[$result, $err] = $client->Basho(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->Sumo(null)->load(
    ["id" => "test01"], null
);
