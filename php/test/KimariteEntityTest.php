...
class KimariteEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = SumoSDK::test(null, null);
        $ent = $testsdk->Kimarite(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Kimarite(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Kimarite(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = kimarite_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "kimarite." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SUMO_TEST_KIMARITE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $kimarite_ref01_data = null;
        if (count($kimarite_ref01_data_raw) > 0) {
...
function kimarite_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/kimarite/KimariteTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["kimarite01", "kimarite02", "kimarite03"] as $k) {
    ...
    $entid_env_raw = getenv("SUMO_TEST_KIMARITE_ENTID");
