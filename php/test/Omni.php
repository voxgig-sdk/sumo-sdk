...
//    only - a typed SumoContext there would make every ctx
...
final class SumoOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SumoContext($clean, null);
    ...
    public static function err_from_map($m): ?SumoError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SumoError(is_string($code) ? $code : '', $msg);
    }
