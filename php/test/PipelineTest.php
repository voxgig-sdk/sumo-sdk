...
{
    private static function utility(): SumoUtility
    {
        return new SumoUtility();
    }
    ...
    private static function ctx(array $over = []): SumoContext
    {
        ...
        $ctx->op = new SumoOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SumoError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SumoOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SumoMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SumoOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SumoMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SumoOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SumoMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SumoSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SumoMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SumoResponse([]),
            'result' => new SumoResult([]),
        ]);
        [, $err] = SumoMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SumoSpec(['step' => 's']),
            'response' => new SumoResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SumoResult([]),
        ]);
        [, $err] = SumoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SumoSpec(['step' => 's']),
            'response' => new SumoResponse(self::resp(200, ['v' => 1])),
            'result' => new SumoResult([]),
        ]);
        [, $err] = SumoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SumoSpec(['step' => 's']),
            'response' => new SumoResponse(self::resp(200, ['v' => 2])),
            'result' => new SumoResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SumoMakeResponse::call($ctx);
    ...
    {
        $preset = new SumoResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SumoSpec([]),
            'response' => new SumoResponse([]),
            'result' => new SumoResult([]),
        ...
        [$got, $err] = SumoMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SumoResult([])]);
        [, $err] = SumoMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SumoSpec([]), 'result' => null]);
        [, $err] = SumoMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SumoSpec(['step' => 's']),
            'result' => new SumoResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SumoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SumoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SumoSpec(['step' => 's']),
            'result' => new SumoResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SumoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SumoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SumoResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SumoSpec([]),
            'result' => new SumoResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SumoMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new SumoError('boom', 'boom');
        $utility->fetcher = function (SumoContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SumoSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SumoMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SumoContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SumoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SumoResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SumoContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SumoSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SumoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SumoResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SumoSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SumoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SumoMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SumoResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SumoDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SumoResult(['ok' => false])]);
        $this->expectException(SumoError::class);
        SumoDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SumoResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SumoMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SumoBaseFeature();
        $b = new SumoBaseFeature();
        SumoFeatureAdd::call($ctx, $a);
        SumoFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SumoBaseFeature
    {
        $f = new SumoBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): SumoContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SumoSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new SumoResponse(['headers' => null]),
            'result' => new SumoResult([]),
        ]);
        SumoResultHeaders::call($ctx);
