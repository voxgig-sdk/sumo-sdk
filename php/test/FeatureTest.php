...
class FtCtrl extends SumoControl
{
...
class FtClient
{
    ...
    private SumoUtility $utility;

    public function __construct(SumoUtility $utility, array $options)
    {
    ...
    public function get_utility(): SumoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SumoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SumoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SumoConfig::make_feature($name);
    ...
    private static function buildUrl(SumoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SumoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SumoResponse($fetched);
        }
    ...
    private function fail(SumoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SumoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
