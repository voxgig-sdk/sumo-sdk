...
local RikishiEntity = {}
RikishiEntity.__index = RikishiEntity


function RikishiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RikishiEntity)
  self._name = "rikishi"
  self._client = client
...
function RikishiEntity:get_name()
  return self._name
end


function RikishiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RikishiEntity.new(self._client, opts)
end
...
function RikishiEntity:mark_deleted()
  self._deleted = true
end


function RikishiEntity:deleted()
  return true == self._deleted
end


function RikishiEntity:data_set(args)
  if args ~= nil then
...
function RikishiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RikishiEntity:match_set(args)
  if args ~= nil then
...
function RikishiEntity:match_get()
...
function RikishiEntity:stream(action, args, callopts)
...
function RikishiEntity:_run_op(ctx, post_done)
  local utility = self._utility
