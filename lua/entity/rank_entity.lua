...
local RankEntity = {}
RankEntity.__index = RankEntity


function RankEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RankEntity)
  self._name = "rank"
  self._client = client
...
function RankEntity:get_name()
  return self._name
end


function RankEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RankEntity.new(self._client, opts)
end
...
function RankEntity:mark_deleted()
  self._deleted = true
end


function RankEntity:deleted()
  return true == self._deleted
end


function RankEntity:data_set(args)
  if args ~= nil then
...
function RankEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RankEntity:match_set(args)
  if args ~= nil then
...
function RankEntity:stream(action, args, callopts)
...
function RankEntity:_run_op(ctx, post_done)
  local utility = self._utility
