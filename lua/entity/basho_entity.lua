...
local BashoEntity = {}
BashoEntity.__index = BashoEntity


function BashoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BashoEntity)
  self._name = "basho"
  self._client = client
...
function BashoEntity:get_name()
  return self._name
end


function BashoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BashoEntity.new(self._client, opts)
end
...
function BashoEntity:mark_deleted()
  self._deleted = true
end


function BashoEntity:deleted()
  return true == self._deleted
end


function BashoEntity:data_set(args)
  if args ~= nil then
...
function BashoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BashoEntity:match_set(args)
  if args ~= nil then
...
function BashoEntity:match_get()
...
function BashoEntity:stream(action, args, callopts)
...
function BashoEntity:_run_op(ctx, post_done)
  local utility = self._utility
