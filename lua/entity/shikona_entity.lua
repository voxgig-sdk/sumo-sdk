...
local ShikonaEntity = {}
ShikonaEntity.__index = ShikonaEntity


function ShikonaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShikonaEntity)
  self._name = "shikona"
  self._client = client
...
function ShikonaEntity:get_name()
  return self._name
end


function ShikonaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShikonaEntity.new(self._client, opts)
end
...
function ShikonaEntity:mark_deleted()
  self._deleted = true
end


function ShikonaEntity:deleted()
  return true == self._deleted
end


function ShikonaEntity:data_set(args)
  if args ~= nil then
...
function ShikonaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShikonaEntity:match_set(args)
  if args ~= nil then
...
function ShikonaEntity:match_get()
...
function ShikonaEntity:stream(action, args, callopts)
...
---@param reqmatch ShikonaListMatch
---@param ctrl? table
---@return Shikona[]
---@return string? err
function ShikonaEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ShikonaEntity:_run_op(ctx, post_done)
  local utility = self._utility
