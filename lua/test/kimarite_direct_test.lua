...
describe("KimariteDirect", function()
  it("should direct-list-kimarite", function()
    local setup = kimarite_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-kimarite", setup.live and "live" or "unit")
  ...
  it("should direct-load-kimarite", function()
    local setup = kimarite_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-kimarite", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function kimarite_direct_setup(mockres)
