...
The Lua SDK for the Sumo API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Basho(nil):list(nil, nil)
...
```lua
local result, err = client:Basho(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Sumo(nil):load(
  { id = "test01" }, nil
)
