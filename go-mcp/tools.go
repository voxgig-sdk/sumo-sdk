...
type Args struct {
	Entity string         `json:"entity" jsonschema:"basho | kimarite | measurement | rank | rikishi | shikona"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.SumoSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "sumo_list",
		Description: "List records from Sumo. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "sumo_load",
		Description: "Load a single record from Sumo. " +
...
func runOp(client *sdk.SumoSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.SumoSDK, name string) (sdk.SumoEntity, error) {
	switch strings.ToLower(name) {
	case "basho":
		return client.Basho(nil), nil
	case "kimarite":
		return client.Kimarite(nil), nil
	case "measurement":
		return client.Measurement(nil), nil
	case "rank":
		return client.Rank(nil), nil
	case "rikishi":
		return client.Rikishi(nil), nil
	case "shikona":
		return client.Shikona(nil), nil
