...
[MCP](https://modelcontextprotocol.io) server exposing the Sumo SDK as
two agent tools — `sumo_list` and `sumo_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sumo-mcp)
make build

# 2. Provide credentials via the environment
export SUMO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sumo \
  -- /absolute/path/to/sumo-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sumo-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sumo_list: first page of records
{ "entity": "basho" }
{ "entity": "basho", "query": { } }

// sumo_load: one record by id
{ "entity": "basho", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sumo-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sumo \
     -- "$PWD"/dist/*/sumo-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sumo_list` and `sumo_load` tools now appear
   in new sessions. Ask the agent to *"list basho using sumo"*
   and it calls `sumo_list` with `{"entity":"basho"}`.
...
```sh
export SUMO_APIKEY=sk_live_xxx            # API key
export SUMO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `sumo_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sumo_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SUMO_APIKEY` | API key sent with every request. |
| `SUMO_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

basho | kimarite | measurement | rank | rikishi | shikona

### Smoke test via HTTP (raw JSON-RPC)

```sh
./sumo-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sumo_load","arguments":{"entity":"basho","query":{"id":1}}}}'
```
