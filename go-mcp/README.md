...
MCP server exposing the Sumo SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `sumo_list` | `entity`, optional `query` map | First page of records as JSON |
| `sumo_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `sumo_list` and
`sumo_load` tools become available in new sessions.
