...
class RikishiEntity
  ...
  def make
    opts = @_entopts.dup
    RikishiEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Rikishi.load works with no args).
  ...
  #
  # @param reqmatch [RikishiListMatch, Hash, nil] match filter (any subset of
  #   Rikishi fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
