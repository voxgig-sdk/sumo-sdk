...
class BashoEntity
  ...
  def make
    opts = @_entopts.dup
    BashoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SumoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Basho, Hash] the current Basho data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SumoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Basho fields)
  def match_get
  ...
  # Load a single Basho.
  #
  # @param reqmatch [BashoLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Basho.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Basho, Hash] the loaded Basho; raises SumoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SumoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BashoListMatch, Hash, nil] match filter (any subset of
  #   Basho fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Basho>, Array] the matching Basho items; raises SumoError on failure
