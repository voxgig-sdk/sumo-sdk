...
class ShikonaDirectTest < Minitest::Test
  def test_direct_list_shikona
    setup = shikona_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-shikona", setup[:live] ? "live" : "unit")
    if _should_skip
...
def shikona_direct_setup(mockres)
