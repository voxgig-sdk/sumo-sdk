...
class MeasurementDirectTest < Minitest::Test
  def test_direct_list_measurement
    setup = measurement_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-measurement", setup[:live] ? "live" : "unit")
    if _should_skip
...
def measurement_direct_setup(mockres)
