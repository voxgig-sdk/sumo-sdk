...
func TestBashoDirect(t *testing.T) {
	t.Run("direct-list-basho", func(t *testing.T) {
		setup := bashoDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-basho", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"basho01", "day01", "division01"} {
	...
	t.Run("direct-load-basho", func(t *testing.T) {
		setup := bashoDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-basho", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["basho_id"] = setup.idmap["basho01"]
			listParams["day"] = setup.idmap["day01"]
			listParams["division"] = setup.idmap["division01"]
...
type bashoDirectSetupResult struct {
	client *sdk.SumoSDK
...
func bashoDirectSetup(mockres any) *bashoDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["SUMO_TEST_BASHO_ENTID"]; ok {
		...
		return &bashoDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &bashoDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
