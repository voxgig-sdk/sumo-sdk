...
func TestRikishiEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Rikishi(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Rikishi(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		rikishiRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.rikishi")))
