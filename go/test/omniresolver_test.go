...
import (
	...
	omni "github.com/voxgig-sdk/sumo-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SumoError); is {
				out := map[string]any{
					"name":    "SumoError",
...
func omniCtx(arg any, client *sdk.SumoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SumoSDK); is {
			client = live
