...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("SUMO_TEST_LIVE") == "TRUE" ||
		os.Getenv("SUMO_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("SUMO_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.SumoSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.SumoError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.SumoSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.SumoError{Msg: msg, Code: code}
}
